...
## The EG4S20BG256 timing parameters are not known, since the datasheets seem not to include this information.
## Some code with unknown author was found on the internet for the Sipeed Tang Primer for which some SDRAM parameters are
## coded. This code uses such parameters for testing.
...
---------------------------
Implements the HDL for a simple SDRAM controller for an embedded SDRAM chip like the one found in ANLOGIC fpgas.
...
class SimpleSDRAMController(Elaboratable):
   def __init__(self, systemClockFrequency = 120e6,
      ...
      self.tRC_ns      =     63.0 #Row cycle time (same bank) - ns
      self.tRCD_ns     =     21.0 #RAS to CAS delay (same bank) - ns
      self.tRP_ns      =     21.0 #Precharge to refresh/row activate delay (same bank) - ns
      self.tRRD_ns     =     14.0 #Row activate to row activate delay (different banks)
      self.tRAS_ns     =     42.0 #Row activate to precharge time, minimum (same bank)
      #self.tRASMax_ns  = 120000.0 #Row activate to precharge time, maximum (same bank) - however tREFI comes first and refresh can only be performed with all Banks in Idle
      self.tWR_ns      =      2.0 #Write recovery time
      self.tMRD_ns     =     14.0 #Mode register set cycle time
      ...
      self.pageColumnIndex           = Signal(self.sdramColumnAddressWidth)
      ...
      print('/*');
      print(' * EG4S20BG256 SDRAM controller');
      print(' *');
            ...
            with m.Else():            
               ...
               with m.If(self.cmdIndex == 4):
                  ...
                  if self.nrOfReadWriteSuspendCycles > 0:
                     ...
                     m.d.clkSDRAM += self.sdramDqIn.eq(self.ctrlWrDataIn)
                     with m.If(self.pageColumnIndex < self.pageWords - 1):
                        m.d.clkSDRAM += self.pageColumnIndex.eq(self.pageColumnIndex + 1)
                     with m.Else():
                        m.d.clkSDRAM += self.sdramDqWRn.eq(0) #End write by making sure that we tri-state the input latch
